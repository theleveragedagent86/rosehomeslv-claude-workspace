...
**Use the Slug Registry for all internal links.** The Organizer Agent pre-assigns a verified unique slug to every topic. Content Producers must ONLY use slugs from the Slug Registry — never invent new slugs.

Link format: `[Post Title](https://www.rosehomeslv.com/blogs/[slug-from-registry])`
...
Create one document per batch of 5 posts. See [seo-guidelines.md](seo-guidelines.md) for full SEO/AEO/GEO strategy.
...
**Slug** (from Slug Registry — do NOT create your own):
[exact slug assigned by Organizer Agent]
```
